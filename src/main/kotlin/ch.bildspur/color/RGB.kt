 ...
 */
data class RGB(@Expose val r: Int, @Expose val g: Int, @Expose val b: Int, @Expose val a: Float = 1f) : ConvertibleColor {
    companion object {
        /**
    ...
    // math
    operator fun plus(v: Float) = RGB((r + v).clamp(), (g + v).clamp(), (b + v).clamp())
    operator fun minus(v: Float) = RGB((r - v).clamp(), (g - v).clamp(), (b - v).clamp())
    operator fun times(v: Float) = RGB((r * v).clamp(), (g * v).clamp(), (b * v).clamp())
    operator fun div(v: Float) = RGB((r / v).clamp(), (g / v).clamp(), (b / v).clamp())

    operator fun plus(v: RGB) = RGB((r + v.r).clamp(), (g + v.g).clamp(), (b + v.b).clamp())
    operator fun minus(v: RGB) = RGB((r - v.r).clamp(), (g - v.g).clamp(), (b - v.b).clamp())
    operator fun times(v: RGB) = RGB((r * v.r).clamp(), (g * v.g).clamp(), (b * v.b).clamp())
    operator fun div(v: RGB) = RGB((r / v.r).clamp(), (g / v.g).clamp(), (b / v.b).clamp())
}

private fun Int.clamp() = clamp(this, 0, 255)
private fun Float.clamp() = clamp(this, 0f, 1.0f)
